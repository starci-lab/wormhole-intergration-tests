import { Account, Aptos, AptosConfig, Ed25519PrivateKey, Network, sleep } from "@aptos-labs/ts-sdk"
...
import { nativeTokenId, signSendWait, wormhole, WormholeMessageId } from "@wormhole-foundation/sdk"
...
describe("Sould Aptos to Solana work", () => {
    it("Should complete with native token transfer", async () => {
        ...
        const aptosAccount = Account.fromPrivateKey({
            privateKey: new Ed25519PrivateKey("0x20c324e81735323b34ed79b28136bee01ae08e2b977291c3399a51a9c81b101d"),
            address: "0x14baf48e4826e1470890519d51efab9918e33b1663f4aaa7b8acf4e86293edca"
        })
        ...
        const solanaAccount = Keypair.fromSecretKey(Buffer.from("e8d6d56a61033f825df1ff28b45ae47c10c613091566ec21db2998302677f9c9e0b32b877ac1d3cd1cb468991605967640c53986bb3598a29fd6652541cab2f0", "hex"))
        const solanaClient = new Connection(clusterApiUrl("devnet"), "confirmed");

        //ensure solana balance > 0
        const solanaBalance = await solanaClient.getBalance(solanaAccount.publicKey)
        ...
        const aptosSigner = new AptosSigner("Aptos", _aptosAccount, _aptosClient, true)
        const solanaSigner = new SolanaSigner("Solana", solanaAccount, solanaClient, true)

        //get aptos bridge
        const amount = 0.01 // 0.01 APT
        const wh = await wormhole("Testnet", [solana, aptos])
        const decimals = await wh.getDecimals("Aptos", nativeTokenId("Aptos").address)
        ...
        const txGenerator = aptosTokenBridge.transfer(
            ...
            //recipient
            {
                address: new SolanaAddress(solanaAccount.publicKey.toString()),
                chain: "Solana"
            },
            //token,
        ...
        const transactionIds = await signSendWait(aptosChain, txGenerator, aptosSigner)
        ...
        const txGenerator2 = solanaTokenBridge.redeem(
            //signer
            new SolanaAddress(solanaAccount.publicKey.toString()),
            //recipient
            vaa,
        )
        const transactionIds2 = await signSendWait(solanaChain, txGenerator2, solanaSigner)
